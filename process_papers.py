...
def generate_latex_summary(text):
    ...
    system_prompt = (
        ...
        "STRICT LATEX RULES:\n"
        "1. DO NOT double-escape backslashes. Output \\begin, not \\\\begin.\n"
        "2. For table row breaks, use exactly \\\\, not \\\\\\\\.\n"
        "3. If you generate a tabular environment, the column count must EXACTLY match the '&' separators in every row.\n"
        "4. Do not use \\multicolumn.\n"
        "5. Escape standalone ampersands in text as \\&.\n\n"
        "\\papersummary\n"
        "{Title}\n"
        "{Authors}\n"
        "{Year, Venue}\n"
...
def main():
    ...
    for pdf in pdfs:
        ...
        try:
            ...
            # 1. Strip markdown formatting if AI still wraps the output
            ...
            # 2. Force-correct over-escaped backslashes commonly generated by LLMs
            latex_block = latex_block.replace(r"\\begin", r"\begin")
            latex_block = latex_block.replace(r"\\end", r"\end")
            latex_block = latex_block.replace(r"\\hline", r"\hline")
            latex_block = latex_block.replace(r"\\toprule", r"\toprule")
            latex_block = latex_block.replace(r"\\midrule", r"\midrule")
            latex_block = latex_block.replace(r"\\bottomrule", r"\bottomrule")
            latex_block = latex_block.replace(r"\\papersummary", r"\papersummary")
            
            # Fix quadruple backslashes intended for row breaks
            latex_block = latex_block.replace(r"\\\\", r"\\")
            # -------------------------------
